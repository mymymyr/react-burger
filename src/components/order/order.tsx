...
import { type TIngredient, type TOrder } from '../../types';
...
const Order: FC<TOrderProps> = ({ order, openModal, displayStatus }) => {
  ...
  const prepareIngredients = useMemo(() => {
    if (storeIngredients.length > 0) {
      return orderIngredients.reduce((total: TIngredient[], id: string) => {
        const ing = storeIngredients.find(el => el._id === id);
        if (ing == null) {
          return total;
        } else if (ing.type === BURGER_COMPOSITION.bun && ((total.length > 0 && total[0].type !== BURGER_COMPOSITION.bun) || total.length === 0)) {
          total.unshift({ ...ing });
  ...
  const bun = useMemo(() => { return ingredients.find((item: TIngredient) => item.type === BURGER_COMPOSITION.bun); }, [ingredients]);
  const internalIngredients = useMemo(() => ingredients.filter((item: TIngredient) => item.type !== BURGER_COMPOSITION.bun), [ingredients]);
  const sum = useMemo(() => (bun ? bun.price * 2 : 0) +
        internalIngredients.reduce((prev: number, cur: TIngredient) => prev + cur.price, 0), [internalIngredients, bun]);
              ...
              : <>
                    ...
                    <div className={`${orderStyles.container} text text_type_digits-default`}>
                        <div className={orderStyles.images}>
                            {ingredients.slice(0, MAX_COUNT + 1).map((el: TIngredient, index: number) => {
